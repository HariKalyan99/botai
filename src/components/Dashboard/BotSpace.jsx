...
function BotSpace(props) {
  ...
  const prompts = [
    ...
    },
    {
      ...
      "response": "Its Summer, I would like to connect you with someone who might be better able to answer your question - copy this link and paste it on your browser : https://tronics-weatherapp.vercel.app/"
    },{
      "id": 51,
      ...
      "response": "Its 38 deg, I would like to connect you with someone who might be better able to answer your question - copy this link and paste it on your browser : https://tronics-weatherapp.vercel.app/"
    },
  ...
  const handleGeo = async() => {
    try{
      ...
      localStorage.setItem('geo', JSON.stringify({city: data.city, country: data.country}))
      handleWeather();
    }catch(error){
      console.log("Error", error)
    }
  ...
  React.useEffect(() => {
    ...
    if(getQuestion === "Hi, what is the temperature" && getQuestion.length > 0){
      ...
      let city = JSON.parse(localStorage.getItem('geo'));
      let promtQuestion = promtList.find(x => x.question === getQuestion);
      setQuestionAnswers([...questionAnswers, {...promtQuestion, response: `Your location is ${city.city}, ${city.country}`,time: now.toLocaleString(), rating: 0, feedBack: ""}]);
      handlePageChange("newPromt")
    }
      ...
      if(promtQuestion){
        ...
        setQuestionAnswers([...questionAnswers, { id: uuidV4(), question: getQuestion, response: `I am unable to fulfill this request, kindly ask questions from this list: ${prompts.map((pr, ind) => `${ind+1})${pr.question}`)}`, time: now.toLocaleString(), rating: 0, feedBack: ""}]);
        handlePageChange("newPromt")
      }
    }
  }, [getQuestion])
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        {showPage === "defaultPromts" ? <DefaultPromts handleGeo={handleGeo} entryText={"How Can I Help You Today?"} handleQuestion={handleQuestion} /> : showPage === "newPromt" ? <Newpromts saveToLocal={saveToLocal} questionAnswers={questionAnswers} handleQuestion={handleQuestion} handleFeedBack={handleFeedBack}
